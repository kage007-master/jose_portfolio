...
export default function MyLink({ name, Icon, border, url }: Props) {
  return (
    ...
    >
      <span className="group-hover:mx-2 group-hover:text-primary transition-all duration-300">
        {name}
      </span>
      <Icon className="text-3xl group-hover:text-primary transition-all duration-300" />
    </a>
  );
}
