...
export default function WorkLb({ workId, setWorkId }: Props) {
  const item = worksData.find((data) => data.id === workId);
        ...
        <div className="w-full h-full max-h-full overflow-y-scroll myScroll">
          <div className="w-full relative h-[45rem]">
            <AiFillCloseCircle
              ...
              className="absolute z-10 text-5xl cursor-pointer top-6 right-8 text-primary"
            />
            <Image
              src={item.images[0]}
          ...
          <section className="py-14 px-14">
            ...
            <ul className="grid grid-cols-1 mt-10 sm:grid-cols-2 gap-y-2">
              <MyInfo field="created by" value={item.ownerName} />
              <MyInfo
                field="date"
                value={new Date(item.date).toLocaleDateString()}
              />
              <MyInfo field="client" value={item.clientName} />
            ...
            {item.techStack.length && item.userActions.length && (
